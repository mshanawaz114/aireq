...
// classification) is close enough to Claude Haiku to be acceptable; we switch
// to Anthropic by flipping LLM__PROVIDER=anthropic when there's revenue.
//
// Model mapping:
//   LlmModel.Haiku   → llama-3.1-8b-instant  (default; overridable via env)
//   LlmModel.Sonnet  → llama-3.3-70b-versatile (default; overridable via env)
//
// Config keys:
//   GROQ_API_KEY           — required at first call
//   GROQ_MODEL_FAST        — override the Haiku-tier model name
//   GROQ_MODEL_STRONG      — override the Sonnet-tier model name
//   LLM__BUDGET__HAIKU__*  — same per-tenant caps as Anthropic
//
// Refs: AIRMVP1-105
    ...
    ILogger<GroqLlmGateway> log) : ILlmGateway
{
    ...
    public async Task<LlmResponse> CompleteAsync(LlmRequest request, CancellationToken ct = default)
    {
        ...
        // ---- 1. Budget check (same shape as the Anthropic gateway) ----------
        ...
        // ---- 2. Issue HTTP call (OpenAI-compatible Chat Completions) --------
        ...
        if (!httpResp.IsSuccessStatusCode)
        {
            ...
            log.LogError(
                "Groq call failed: {Status} {Reason} — body={Body}",
        ...
        // Groq is free at MVP volume; cost estimate is mostly bookkeeping.
        // Use the configured per-million rates (default 0 in the free profile).
        ...
        log.LogInformation(
            "LLM call OK (Groq) — tenant={TenantId} model={Model} purpose={Purpose} " +
